...
import com.oakiha.audiobookplayer.data.media.ImageCacheManager
import com.oakiha.audiobookplayer.data.media.MetadataEditError
...
import com.oakiha.audiobookplayer.utils.FileDeletionUtils
    ...
    suspend fun saveMetadata(
        ...
        val finalCoverArtUpdate = if (coverArtUpdate == null) {
            val existingMetadata = try {
                 com.oakiha.audiobookplayer.data.media.AudioMetadataReader.read(java.io.File(song.path))
            } catch (e: Exception) {
                null
            }
